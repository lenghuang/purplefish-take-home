...
describe('Main Application Flow - Characterization Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockTemplateManager = {
      getAllTemplates: jest.fn().mockReturnValue([
        { id: 1, name: 'Software Engineer', role: 'Software Engineer', steps: ['step1', 'step2'] },
      ]),
    ...
    mockCLI = {
      selectTemplateDropdown: jest.fn().mockResolvedValue({ id: 1, name: 'Software Engineer', role: 'Software Engineer', steps: ['step1', 'step2'] }),
      addMessage: jest.fn(),
    ...
    mockAgentService = {
      ...
      addMessage: jest.fn(),
      generateResponse: jest.fn().mockResolvedValue({ content: 'Next question?' }),
    };
  ...
  describe('main application flow', () => {
    ...
    it('should create agent service with correct configuration', () => {
      ...
      new AgentService(agentConfig);
      expect(AgentService).toHaveBeenCalledWith(agentConfig);
    });
    ...
    it('should initialize conversation correctly', async () => {
      ...
      expect(mockAgentService.initializeConversation).toHaveBeenCalledWith(userId, templateId);
    });

    it('should handle the interview loop structure', async () => {
      // This captures the current loop structure expectations
      const selectedTemplate = { id: 1, name: 'Software Engineer', role: 'Software Engineer', steps: ['step1', 'step2'] };
      ...
      expect(mockAgentService.addMessage).toHaveBeenCalledWith(conversationId, 'user', userInput);
    });
