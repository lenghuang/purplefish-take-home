...
describe('AgentService - Characterization Tests', () => {
  ...
  beforeEach(() => {
    ...
    jest.doMock('../db/repositories/messageRepository', () => ({
      MessageRepository: jest.fn().mockImplementation(() => ({
        create: jest.fn().mockResolvedValue({ id: 1, content: 'test', sender: 'user' }),
  ...
  describe('initializeConversation', () => {
    it('should create a conversation with correct data structure', async () => {
      ...
      agentService['conversationRepo'].create = jest.fn().mockResolvedValue(mockConversation);
  ...
  describe('addMessage', () => {
    it('should add message with correct structure', async () => {
      ...
      agentService['messageRepo'].create = jest.fn().mockResolvedValue(mockMessage);
  ...
  describe('getConversationContext', () => {
    it('should return null for non-existent conversation', async () => {
      agentService['conversationRepo'].findById = jest.fn().mockResolvedValue(null);
    ...
    it('should return conversation context with messages', async () => {
      ...
      agentService['conversationRepo'].findById = jest.fn().mockResolvedValue(mockConversation);
      agentService.getMessagesForConversation = jest.fn().mockResolvedValue(mockMessages);
