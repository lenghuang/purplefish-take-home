...
function extractStructuredData(text: string, currentState: InterviewState): Partial<InterviewState> {
  ...
  switch (currentState.stage) {
    ...
    case "basic_info":
      ...
      const namePatterns = [/(?:my name is|i'm|i am|call me)\s+([a-zA-Z\s]+)/i, /^([a-zA-Z\s]{2,30})$/]
    ...
    case "salary_negotiation":
      if (
        ...
        updates.salaryAcceptable = true
        updates.stage = "license_check"
      } else if (lowerText.includes("no") || lowerText.includes("can't") || lowerText.includes("won't")) {
        updates.endedEarly = true
        updates.completed = true
    ...
    case "license_check":
      ...
      } else if (lowerText.includes("no") || lowerText.includes("not licensed") || lowerText.includes("don't have")) {
    ...
    case "experience":
      if (lowerText.includes("yes") || lowerText.includes("year") || lowerText.includes("experience")) {
      ...
      } else if (lowerText.includes("no") || lowerText.includes("don't have") || lowerText.includes("new grad")) {
    ...
    case "alternative_experience":
      if (lowerText.includes("yes") || lowerText.includes("acute") || lowerText.includes("hospital")) {
        updates.stage = "experience_details"
      } else {
        updates.stage = "completed"
        updates.completed = true
...
function getFallbackResponse(state: InterviewState): Response {
  const fallbackResponses = {
    ...
    experience_details: "Wonderful! Can you tell me about a challenging emergency situation you handled in the ICU?",
